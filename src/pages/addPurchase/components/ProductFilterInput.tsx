...
import type { Product } from "../../../hooks/usePurchaseForm";
import { useAddPurchaseStore, useDepartmentStore } from "../../../stores";
import { FaSearch } from "react-icons/fa";

// --- MAIN FILTERED INPUT COMPONENT ---
interface Props {
  value: string;
  items: Product[];
  placeholder?: string;
...
const ProductFilterInput = ({
  ...
  const getDepartmentById = useDepartmentStore((s) => s.getItemById);
  const getDepartmenName = (department_id: number) =>
    getDepartmentById(department_id).name;

  const getCategoryName = (department_id: number, category_id: number) =>
    getDepartmentById(department_id).categories.find(
      (c) => c.id === category_id
    )?.name;
  ...
  useEffect(() => {
    const existingProduct = purchaseItems.find(
      (item) => item.product_data.name === value
    );
    if (existingProduct) {
  ...
  }, [editItem, items, onSelectItem, purchaseItems, value]);

  return (
    <div className="relative" ref={componentRef}>
      {/* The main input field container */}
      <div className="flex items-center w-full border rounded-lg shadow-sm transition-all duration-200 border-border-add-purchase text-[var(--color-text-primary-mock)] focus-within:border-[var(--color-primary-mock)] focus-within:shadow-sm">
              ...
              >
                ...
                >
                  ...
                  <span className="text-xs bg-muted-mock123 text-muted-foreground-mock123 px-2 py-1 rounded-full">
                    {" "}
                    {getDepartmenName(item.department_id)} ---&gt;{" "}
                    {getCategoryName(item.department_id, item.category_id)}
                  </span>
                </button>
              </li>
            ))}
