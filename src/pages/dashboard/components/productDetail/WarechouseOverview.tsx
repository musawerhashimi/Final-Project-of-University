...
function StockOverview({ productId }: Props) {
  ...
  const warehouses = getWarehouses ? getWarehouses() : [];
  const [selectedLocationId, setSelectedLocationId] = useState<number>(
    warehouses.length > 0 ? warehouses[0].id : 1
  );
      ...
      <div className="bg-blue-50 rounded-lg shadow-inner mx-auto">
        <div className="space-y-0">
          {[1, 2, 3, 4].map((i) => (
            ...
            >
              <div className="p-3 w-1/2 bg-blue-50">
                <div className="h-5 bg-gray-200 rounded animate-pulse"></div>
              </div>
              <div className="p-3 w-1/2 bg-green-50">
                <div className="h-5 bg-gray-200 rounded animate-pulse"></div>
              </div>
            </div>
          ))}
  ...
  return (
    <div className="">
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg mb-8 border border-gray-200">
        ...
        >
          {t("Select Warehouse to see Overview")}
        </label>
        <select
          id="stock-select"
          ...
          <div className="mb-8 relative">
            <h1 className="text-center text-gray-800  md:text-3xl print:text-xl font-extrabold p-2 mb-2">
              ...
              <span className="text-blue-500">
                {warehouses.find((s) => s.id === selectedLocationId)?.name ||
                  t("Warehouse")}
              </span>
            </h1>
            {/* PDF Button */}
