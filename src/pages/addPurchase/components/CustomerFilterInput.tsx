...
interface Props {
  ...
  onSelectItem: (item: SaleCustomer) => void;
}
    ...
    <div className="relative" ref={componentRef}>
      ...
      <div className="flex items-center w-full border rounded-lg shadow-sm transition-all duration-200 border-border-color text-[var(--color-text-primary-mock)] focus-within:border-[var(--color-primary-mock)] focus-within:shadow-sm">
        ...
        <input
          ...
          // className="w-full h-12 py-2 pr-4 bg-transparent text-card-foreground-mock123 placeholder-muted-foreground-mock123 focus:outline-none"
      ...
      {isListOpen && items.length > 0 && (
        <div className="absolute top-full w-full origin-top shadow-md rounded-lg bg-surface z-10 bg-popover-mock123 overflow-hidden">
          <ul className=" flex flex-wrap py-1 max-h-80 overflow-y-auto">
            {items.map((item) => (
              <li
                key={item.id}
                className="hover:bg-sky-100 hover:text-black border-r w-full last:border-r-0 border-border-color"
              >
                <button
...
export default CustomerFilterInput;
